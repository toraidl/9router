...
const getConfigDir = () => path.join(os.homedir(), ".config", "mimocode");
const getConfigPath = () => path.join(getConfigDir(), "mimocode.json");

// Check if mimo CLI is installed (via which/where or config file exists)
...
// GET - Check mimo CLI and read current settings
  ...
  try {
    ...
    if (!isInstalled) {
      return NextResponse.json({
        installed: false,
        config: null,
        message: "MiMo CLI is not installed",
      });
    }
    ...
    console.log("Error checking mimo settings:", error);
    return NextResponse.json({ error: "Failed to check mimo settings" }, { status: 500 });
  }
}
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "MiMo settings applied successfully!",
      configPath,
    });
  } catch (error) {
    console.log("Error applying mimo settings:", error);
...
export async function PATCH(request) {
  try {
    ...
    console.log("Error patching mimo settings:", error);
...
export async function DELETE(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "9Router settings removed from MiMo",
    });
  } catch (error) {
    console.log("Error resetting mimo settings:", error);
    return NextResponse.json({ error: "Failed to reset mimo settings" }, { status: 500 });
  }
}
