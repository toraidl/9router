...
export default function MimoToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    if (status?.mimo?.models) {
      setSelectedModels(status.mimo.models);
    }
    if (status?.mimo?.activeModel) {
      setActiveModel(status.mimo.activeModel);
    }
  ...
  const saveModels = async (models) => {
    try {
      ...
      await fetch("/api/cli-tools/mimo-settings", {
        method: "POST",
  ...
  const checkStatus = async () => {
    setChecking(true);
    try {
      const res = await fetch("/api/cli-tools/mimo-settings");
      const data = await res.json();
      setStatus(data);
    } catch (error) {
  ...
  const handleReset = async () => {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/cli-tools/mimo-settings", { method: "DELETE" });
      const data = await res.json();
      if (res.ok) {
  ...
  const getManualConfigs = () => {
    ...
    return [{
      filename: "~/.config/mimocode/mimocode.json",
  ...
  return (
    <Card padding="xs" className="overflow-hidden">
              ...
              <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-yellow-600 dark:text-yellow-400">MiMo CLI not detected locally</p>
                ...
                <div className="p-4 bg-surface border border-border rounded-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-black/5 dark:bg-white/5 rounded font-mono text-xs">npm install -g @mimo-ai/mimo</code>
                    </div>
                    <p className="text-text-muted">After installation, run <code className="px-1 bg-black/5 dark:bg-white/5 rounded">mimo</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
                          ...
                          >
                            ...
                            <button
                              onClick={async (e) => {
                                e.stopPropagation();
                                try {
                                  const res = await fetch(`/api/cli-tools/mimo-settings?model=${encodeURIComponent(model)}`, { method: "DELETE" });
                                  if (res.ok) {
      ...
      <ModelSelectModal
        ...
        activeProviders={activeProviders}
        modelAliases={modelAliases}
        title="Select Subagent Model for MiMo Code"
      />
